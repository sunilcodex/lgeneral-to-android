...
namespace Miscellaneous
{
	...
	public class Misc
	{
		...
		{
			int h = 0; 
			if (Misc.IsEven (apro_x)) {
			
				h = -Mathf.RoundToInt (yclick / Config.hex_h);
			} else {
				h = -Mathf.RoundToInt ((yclick + Config.hex_y_offset) / Config.hex_h);
			
			}
			return h;
		}

		public static int GetWidthtPosition (float xclicked)
		{
			int w1 = Mathf.RoundToInt (xclicked / Config.hex_x_offset);
