...
*/
        ...
        */
		...
		static bool ai_eval_ctrl (int x, int y, object _ctx)
		{
			...
			case 0:
				Engine.map.mask [x, y].ctrl_grnd += ctx.score;
				break;
			case 1:
				Engine.map.mask [x, y].ctrl_air += ctx.score;
				break;
			case 2:
				Engine.map.mask [x, y].ctrl_sea += ctx.score;
				break;
			}
			return true;
		}
			...
			if (ai_status != AI_STATUS.AI_STATUS_END) {
				foreach (Unit unit_aux in ai_units) {
					...
					for (i = 0; i < partner_count; i++)
						if (best == null)
							best = partners [i];
						else if (best.str < partners [i].str)
							best = partners [i];
					if (best != null) {
#if DEBUG
				...
				foreach (Unit unit_aux in ai_units) {
					...
					if ((unit_aux.CheckLowFuel () || unit_aux.CheckLowAmmo ())) {
						if (unit_aux.supply_level > 0) {
							Action.action_queue_supply (unit_aux);
							ai_units.Remove (unit_aux);
#if DEBUG
                                Console.WriteLine("{0} supplies", unit_aux.name);
#endif
							break;
						} else {
#if DEBUG
                                Console.WriteLine("{0} searches depot", unit_aux.name);
#endif
							if (ai_get_dist (unit_aux, unit_aux.x, unit_aux.y, AI_FIND.AI_FIND_DEPOT,
                                                  out dx, out dy, out dist))
							if (ai_approximate (unit_aux, dx, dy, out x, out y)) {
								Action.action_queue_move (unit_aux, x, y);
								ai_units.Remove (unit_aux);
#if DEBUG
                                        Console.WriteLine("{0} moves to {1},{2}", unit_aux.name, x, y);
#endif
								break;
							}
						}
        ...
        */
		public static void ai_finalize ()
		{
			...
			while (Engine.stateMachine.scheduler.IsRunning) {
				...
				//TODO_RR System.Threading.Thread.Sleep(100);
			}
